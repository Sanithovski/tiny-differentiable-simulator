...
/*
  ...
  If initial guess has same active set/clamped dimensions as optimum, it will be
...
boxQPResult boxQP(const MatrixXd& Q, const VectorXd& c, const VectorXd& x0,
  ...
  VectorXd clamped_dims(
      n_dims);  // TODO change this to ints - need to deal with casting when
                // interacting with other VectorXd's
  ...
  for (int iter = 0; iter <= qp_maxIter; iter++) {
    ...
    for (int i = 0; i < n_dims; i++) {
      ...
      if ((approx_eq(x(i), lower(i)) && grad(i) > 0) ||
          (approx_eq(x(i), upper(i)) && grad(i) < 0)) {
        clamped_dims(i) = 1;
        res.v_free(i) = 0;
      }
    }
    ...
    if (iter == 0 || (old_clamped_dims - clamped_dims).sum() != 0) {
      ...
      if (choleskyOfQfree.matrixL().size() >
          0) {  // I'm not sure why this happens, but just in case TODO debug
    ...
    if (res.v_free.all()) {
      ...
      VectorXd search_update_vals =
          -(res.R_free.inverse() * res.R_free.transpose().inverse()) *
              subvec_w_ind(grad_clamped, res.v_free) -
          subvec_w_ind(x, res.v_free);
