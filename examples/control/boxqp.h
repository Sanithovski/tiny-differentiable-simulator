...
struct boxQPResult {
  ...
  VectorXd x_opt;
  VectorXi v_free;
  MatrixXd R_free;  // TODO replace this with Qfreeinv? since that's all we use
                    // this for
};
...
inline VectorXd clamp_to_limits(const VectorXd& x, const VectorXd& lower,
                                const VectorXd& upper) {
  VectorXd x_clamped = upper.cwiseMin(x.cwiseMax(lower));
  return x_clamped;
}

inline double quadCost(const MatrixXd& Q, const VectorXd& c,
                       const VectorXd& x) {
