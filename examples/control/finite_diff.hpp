...
MatrixXd finite_diff_jacobian(std::function<VectorXd(VectorXd)> f, VectorXd x,
                              int out_size) {
  int n_dims = x.size();
...
void finite_diff_vecvec2scalar(std::function<double(VectorXd, VectorXd)> f,
                               const VectorXd& x1, const VectorXd& x2,
                               MatrixXd& out) {
  ...
  for (int i = 0; i < x1.size(); i++) {
    for (int j = 0; j < x2.size(); j++) {
      ...
      m2[j] -= eps;
      out(i, j) =
          (f(p1, p2) - f(m1, p2) - f(p1, m2) + f(m1, m2)) / (4 * eps * eps);
    }
  }
}

void finite_diff_hessian(std::function<double(VectorXd)> f, const VectorXd& x,
                         MatrixXd& out) {
