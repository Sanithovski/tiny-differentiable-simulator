...
double iLQR::init_traj(const VectorXd &x_0, const VecOfVecXd &u_0) {
  ...
  std::fill(cxx.begin(), cxx.end(),
            MatrixXd::Zero(model->x_dims, model->x_dims));
  std::fill(cxu.begin(), cxu.end(),
            MatrixXd::Zero(model->x_dims, model->u_dims));
  std::fill(cuu.begin(), cuu.end(),
            MatrixXd::Zero(model->u_dims, model->u_dims));
  std::fill(Vx.begin(), Vx.end(), VectorXd::Zero(model->x_dims));
  std::fill(Vxx.begin(), Vxx.end(),
            MatrixXd::Zero(model->x_dims, model->x_dims));
  ...
  for (iter = 0; iter < maxIter; iter++) {
    ...
    long int elapsed =
        std::chrono::duration_cast<std::chrono::milliseconds>(now - start)
            .count();
    // std::cout << "compute_derivatives took: " << elapsed/1000. << " seconds."
    // << std::endl;
    ...
    while (!backPassDone) {
      ...
      if (diverge != 0) {
        // std::cout << "backward pass diverged" << std::endl;
    ...
    elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - start)
                  .count();
    // std::cout << "backward pass took: " << elapsed/1000. << " seconds." <<
    // std::endl;
    ...
    if (backPassDone) {
      for (int i = 0; i < Alpha.size(); i++) {
        ...
        for (unsigned int i = 0; i < us.size(); i++) {
          ...
          // if(u_plus_feedforward[i][0] < model->u_min[0] ||
          // u_plus_feedforward[i][0] > model->u_max[0]) { std::cout << "u over
          // limits: " << us[i][0] << ", " << u_plus_feedforward[i][0] << "\n"
          // << k[i] << std::endl;
          // }
        }
        ...
        if (expected > 0) {
          ...
          z = sgn(dcost);
          std::cerr
              << "Warning: non-positive expected reduction. This should not "
                 "occur"
              << std::endl;
        }
        ...
        // which would make this incorrect without reset because first line
        // search affects next one
        xs = x_old;
        us = u_old;
      }
    ...
    elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - start)
                  .count();
    // std::cout << "forward pass took: " << elapsed/1000. << " seconds." <<
    // std::endl;
...
// STEP 4: accept step (or not), print status
  ...
  long int t_total =
      std::chrono::duration_cast<std::chrono::milliseconds>(all_end - all_start)
          .count();
  std::cout << "Total time: " << t_total / 1000. << std::endl;
  std::cout << "compute_derivatives: " << t_compute_deriv << std::endl;
  std::cout << "backward pass: " << t_backward << std::endl;
  std::cout << "forward pass: " << t_forward << std::endl;
  std::cout << "other stuff: "
            << t_total / 1000. - (t_compute_deriv + t_backward + t_forward)
            << std::endl;
#endif
...
double iLQR::forward_pass(const VectorXd &x0, const VecOfVecXd &u) {
  ...
  for (int t = 0; t < T; t++) {
    ...
    if (K.size() > 0)
      u_curr +=
          K[t] *
          (x_new[t] - xs[t]);  // apply LQR control gains after first iteration

    // if(u_curr[0] < model->u_min[0] || u_curr[0] > model->u_max[0]) {
    //   std::cout << "u over limits: " << u[t] << ", " << u_curr << "\n" <<
    //   K[t] << std::endl;
    // }
...
int iLQR::backward_pass() {
  ...
  for (int i = (T - 1); i >= 0; i--) {  // back up from end of trajectory
    ...
    QuuF = cuu[i] +
           (lambda * MatrixXd::Identity(model->u_dims, model->u_dims)) +
           (fu[i].transpose() * Vxx[i + 1] * fu[i]);

    boxQPResult res = boxQP(QuuF, Qu, k[min(i + 1, T - 1)],
                            model->u_min - us[i], model->u_max - us[i]);
    ...
    if (v_free.any()) {
      ...
      MatrixXd Lfree = -(R.inverse() * R.transpose().inverse()) *
                       rows_w_ind(Qux_reg, v_free);
...
void iLQR::output_to_csv(const std::string filename) {
  ...
  fclose(XU);
  std::cout << "Saved iLQR result to " << filename << std::endl;
}
